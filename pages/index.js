...
export default function Home() {
  ...
  const runApex = useCallback(async () => {
    ...
    try {
      ...
      setApexData(json);
            try { const SBU = process.env.NEXT_PUBLIC_SUPABASE_URL, SBK = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY; if (SBU && SBK && Array.isArray(json.trades)) { for (const tr of json.trades) { const firstNum = function(v){ const m = String(v||"").match(/[0-9][0-9,]*\.?[0-9]*/); return m ? parseFloat(m[0].replace(/,/g,"")) : null; }; const row = { id: "apex_" + Date.now() + "_" + Math.random().toString(36).slice(2,7), ticker: String(tr.ticker||"").toUpperCase().replace(/[^A-Z0-9]/g,""), asset_class: tr.assetClass || "CRYPTO", direction: tr.direction || "LONG", entry: firstNum(tr.entryZone || tr.currentPrice), stop_loss: firstNum(tr.stopLoss), target1: firstNum(tr.target1), target2: firstNum(tr.target2), conviction: tr.apexConviction || null, notes: [tr.timeHorizon ? ("Expected hold: " + tr.timeHorizon) : null, tr.catalyst, tr.technicalSetup, tr.macroAlignment, tr.confluence, tr.apexSummary].filter(Boolean).join(" | ") || null, status: "OPEN" }; if (row.ticker && row.entry) { fetch(SBU + "/rest/v1/journal_trades", { method: "POST", headers: { apikey: SBK, Authorization: "Bearer " + SBK, "Content-Type": "application/json" }, body: JSON.stringify(row) }).catch(function(){}); } } } } catch (autoSaveErr) {}
      setPhase("apex-done");
    } catch (e) {
